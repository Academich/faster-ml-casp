...
class VanillaTransformer(nn.Module):
    def __init__(
        ...
        self.src_token_featurizer = TokenEmbedding(
            self.src_vocab_size, self.emb_dim, padding_idx=self.src_pad_token_i
        )
        if self.share_embeddings:
            self.tgt_token_featurizer = self.src_token_featurizer
            assert self.src_vocab_size == self.tgt_vocab_size
        else:
            self.tgt_token_featurizer = TokenEmbedding(
                self.tgt_vocab_size, self.emb_dim, padding_idx=self.tgt_pad_token_i
            )
    ...
    def forward(self, src: LongTensor, tgt: LongTensor):
        ...
        src_pad_mask: torch.Tensor = torch.where(
            src != self.src_pad_token_i, torch.tensor(0.0), torch.tensor(float("-inf"))
        )
        tgt_pad_mask: torch.Tensor = torch.where(
            tgt != self.tgt_pad_token_i, torch.tensor(0.0), torch.tensor(float("-inf"))
        )
        tgt_mask = self.transformer.generate_square_subsequent_mask(
            tgt_seq_len
        ).type_as(tgt_emb)
        tgt_emb = self.transformer(
            src_emb,
            tgt_emb,
            src_mask=None,
            tgt_mask=tgt_mask,
