...
def model_setup(use_gpu=True, **kwargs):
    # use_gpu = False
...
def round_trip_transformer_or_med(retro_model, product_model, test_path):  # X is a list of B smiles
    seen = set()
    with open(test_path) as fs:
        desired_mols_list = [s.strip() for s in fs.readlines()][:1]

    retro_model_batch_sz = 32  # RB
    batches_of_tgt_smis = [desired_mols_list[i:i + retro_model_batch_sz] for i in
                           range(0, len(desired_mols_list), retro_model_batch_sz)]

    small_r_problems_prods = []
    eff_N = []
    round_trip_percent = []
    round_trip_pcs = []
    ...
    for smi_batch in batches_of_tgt_smis:
        ...
        for i, tgt_smi in enumerate(smi_batch):
            ...
            print("rxns_for_tgt_smi", rxns_for_tgt_smi)
            ...
            for j in range(num_of_rxns_for_tgt_smi):
                ...
                print("product_smis_for_rxnj")
                [print(p) for p in product_smis_for_rxnj]
    ...
    print("Av. round-trip, percent ", sum(round_trip_percent) / len(round_trip_percent))
    print("Av. round-trip, pcs ", sum(round_trip_pcs) / len(round_trip_pcs))
    print("Av. eff.N, pcs ", sum(eff_N) / len(eff_N))

    print("Av. prod in reactnts pcs", sum(prod_in_reactnts_pcs) / len(prod_in_reactnts_pcs))
    print("Small_r_problems pcs", len(set(small_r_problems)))


def main(retro_model_config, product_model_config, test_path):
    with open(retro_model_config) as stream:
        try:
            ...
            retro_model = model_setup(**retro_model_kwargs)
    ...
    with open(product_model_config) as stream:
        try:
            ...
            product_model = model_setup(**prod_model_kwargs)
        except yaml.YAMLError as exc:
            print(exc)
        round_trip_transformer_or_med(retro_model, product_model, test_path)


if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    parser.add_argument("--test_path", "-t", type=str, default="data/uspto50k_test/products.txt", help="path to USPTO 50k test")
    ...
    parser.add_argument("--retro_model_config", "-ryml", type=str,
                        default="configs/medusa_default_config.yml", help="path to config of evaluated single-step retrosynthesys model (Medusa or Transformer)")
    args = parser.parse_args()

    args = parser.parse_args()
    main(args.retro_model_config, args.product_model_config, args.test_path)
