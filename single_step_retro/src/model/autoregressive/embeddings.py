...
class PositionalEncoding(nn.Module):
    ...
    def forward(
        self, x: torch.Tensor, offset: torch.LongTensor = torch.LongTensor([0])
    ):
        """
        ...
        seq_len = x.size(1)
        shifts = torch.relu(
            torch.arange(1, seq_len + 1).type_as(x) - offset.type_as(x)
        ).long()  # amounts to self.pe[1: seq_len + 1] if offset is 0
        pe = self.pe[shifts]
        return x + pe
