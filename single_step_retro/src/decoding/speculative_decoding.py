...
from single_step_retro.src.utils.drafting import make_drafts
...
class TranslationInferenceGreedySpeculative:
    ...
    def generate(self, src: "torch.LongTensor") -> "torch.LongTensor":
        ...
        src_pad_mask_inflated = torch.repeat_interleave(
            src_pad_mask, N, dim=0
        )  # (B*N, Ls)
        ...
        generated_tokens = (
            torch.full((B, 1), self.bos_token).type_as(src).long()
        )  # (B, 1)

        # Index of the last non-pad token in the generated sequence
        front_index = torch.zeros_like(generated_tokens)  # (B, 1)

        finished_predictions = torch.full((B, self.max_len), self.pad_token).type_as(
            src
        )  # (B, max_len)
        ...
        while generated_tokens.size(1) < self.max_len:  # while gnrtd_len < self.max_len
            ...
            generated_tokens_present_pad_num = (
                (generated_tokens == self.pad_token).sum(0) == Bc
            ).sum()  # (1,)
            generated_tokens_padded = pad(
                generated_tokens,
            ...
            generated_tokens_padded_inflated = torch.repeat_interleave(
                generated_tokens_padded[:, :-1], N, dim=0
            )  # (Bc*N, Lg + D)
            front_index_inflated = torch.repeat_interleave(
                front_index, N, dim=0
            )  # (Bc*N, 1)

            insertion_indices = (
                front_index_inflated + draft_len_range[:-1] + 1
            )  # (Bc*N, D)

            # Concatenate the already generated sequences with the corresponding draft sequences
            draft_tokens_effective_batch_size = draft_tokens.view(
                draft_tokens.size(0) * draft_tokens.size(1), draft_tokens.size(2)
            )  # (Bc*N, D)
            drafted_seqs = generated_tokens_padded_inflated.scatter(
                dim=1, index=insertion_indices, src=draft_tokens_effective_batch_size
            )  # (Bc*N, Lg + D)

            # Run the decoder and sample from the predicted distributions
            pred_logits = self.model.decode_tgt(
                drafted_seqs, memory_inflated, memory_pad_mask=src_pad_mask_inflated
            )  # (Bc*N, Lg + D, V)
            ...
            pred_tokens = pred_tokens.gather(
                dim=1, index=retrieval_indices
            )  # (Bc*N, D + 1)

            # Find the drafts with the most accepted tokens
            verification = (
                draft_tokens_effective_batch_size == pred_tokens[:, :-1]
            )  # (Bc*N, D)
            accepted_in_drafts = draft_len_range[1:] == verification.cumsum(
                -1
            )  # (Bc*N, D)
            ...
            chosen = torch.gather(
                pred_tokens, 1, draft_i.unsqueeze(-1).expand(Bc, 1, D + 1)
            ).squeeze(1)  # (Bc, D + 1)

            # Mask out the tokens that were not accepted
            decline_pred_tokens = draft_len_range > n_accepted  # (Bc, D + 1)
            chosen_truncated_to_accepted = chosen.masked_fill(
                decline_pred_tokens, self.pad_token
            )  # (Bc, D + 1)
            ...
            generated_tokens = generated_tokens_padded.scatter(
                dim=1, index=insertion_indices, src=chosen_truncated_to_accepted
            )  # (Bc, Lg + D + 1)
            front_index = front_index + n_accepted + 1  # (Bc, 1)

            # Put away the sequences that reached the EOS token. It speeds up the decoding
            sequence_finished = (
                (generated_tokens == self.eos_token).sum(-1).bool()
            )  # (Bc,)
            sequence_indices_finised = sequence_finished.nonzero().ravel()  # (Bc,)
            if (
                sequence_indices_finised.nelement() > 0
            ):  # if there are finished sequences
                sequence_to_continue = ~sequence_finished  # (Bc,)
                sequence_finished_inflated = torch.repeat_interleave(
                    sequence_finished, N, dim=0
                )  # (Bc*N,)
                sequence_to_continue_inflated = ~sequence_finished_inflated  # (Bc*N,)
                query_batch_indices_finished = unfinished_query_batch_indices[
                    sequence_finished
                ]  # (Bc,)

                # Save the finished sequences to the output tensor
                finished_predictions[
                    query_batch_indices_finished, : generated_tokens.size(1)
                ] = generated_tokens[sequence_indices_finised]

                # Here we remove the finished sequences from the batch and B becomes Bc
                unfinished_query_batch_indices = unfinished_query_batch_indices[
                ...
                src_pad_mask_inflated = src_pad_mask_inflated[
                    sequence_to_continue_inflated
                ]
...
def topk_in_each_group(score_1d, length_of_each_group, k, pad=None):
    ...
    start_ind_of_each_group = torch.roll(
        length_of_each_group, 1, dims=-1
    )  # -> (b_size)
    start_ind_of_each_group[0] = 0
    start_ind_of_each_group = start_ind_of_each_group.cumsum(-1).unsqueeze(1)
    # -> (N, 1)

    different_num_of_candidates_in_groups = (
        length_of_each_group == max_len_of_group
    ).sum() != b_size
    if different_num_of_candidates_in_groups:
        ...
        inds_for_2d = start_ind_of_each_group + (
            inds_for_2d % length_of_each_group.unsqueeze(1)
        )
...
class TranslationInferenceBeamSearchSpeculativeBatchedWithoutLeftPads:
    def __init__(
        ...
        self.log_prob_pad = 7.0  # should be more than 0.
        ...
        if drafts_len != draft_len:
            print(
                f"The draft length should be in range [{self.min_draft_len}: {self.max_draft_len}], so it was changed to {drafts_len}"
            )
    ...
    def sample(
        ...
        masked_logits = mask_with_num_logits_according_nucleus(
            pred_logits,
            nucleus=20.0,
        ...
        mask_for_unaccepted_draft_tokens = tmp_range.repeat(
            n_candidates, 1
        ) <= n_accepted.unsqueeze(-1)
        ...
        if not_entirely_excepted_bool.sum().item() > 0:
            ...
            chosen_drafts[not_entirely_excepted_bool] = chosen_drafts[
                not_entirely_excepted_bool
            ].scatter(
                index=n_accepted[not_entirely_excepted_bool].unsqueeze(-1),
                dim=1,
                value=self.bos_token_idx,
            )

        masked_logits[:, :-1, :].scatter_(
            index=chosen_drafts.unsqueeze(-1), dim=2, value=0.0
        )  # the accepted tokens in
        # the drafts can not be leaves of the top n tree of sequences

        # Sampling the top n tree of sequences leaves:
        candts_inds, token_postn, token_inds = torch.nonzero(
            masked_logits, as_tuple=True
        )
        # -> (num)
        ...
        num_of_new_seqs_for_each_in_batch = torch.logical_and(
            low_border <= candts_inds_tmp, candts_inds_tmp < high_border
        ).sum(0)
        # -> (b_size)
        ################################################################################################################
        ...
        already_finished_given_seqs = (
            (previous_roots == self.eos_token_idx).sum(-1).bool()
        )  # -> (num)
        ...
        tail = torch.full((num, 1), 0.0).type_as(drafts)  # -> (num, 1)
        new_seqs = torch.cat((drafts, tail), dim=-1)  # (num, draft_len+1)
        new_seqs.scatter_(
            1, index=token_postn.unsqueeze(-1), src=token_inds.unsqueeze(-1)
        )
        #   -> (num, draft_len + 1)
        ...
        predicted_log_probs = pred_logits.softmax(-1).log()[
            candts_inds
        ]  # -> (num, draft_len + 1, vocab_size)

        new_seqs_log_probs = torch.gather(
            predicted_log_probs, dim=2, index=new_seqs.unsqueeze(-1)
        ).squeeze(-1)
        #    -> (num, draft_len + 1)
        new_seqs_log_probs.masked_fill_(mask_for_tokens_after_the_sampled, 0.0)
        #    -> (num, draft_len + 1)
        new_seqs_log_probs = new_seqs_log_probs.cumsum(
            dim=-1
        )  # -> (num, draft_len + 1)

        last_log_prob_from_roots = torch.min(
            log_prob_of_roots, dim=-1, keepdim=True
        ).values
        # (num, 1)
        ...
        new_seqs_place_bool = torch.logical_or(
            draft_place_bool, torch.roll(draft_place_bool, 1, 1)
        )
        # -> (num, drafted_len) It contains draft_len+1 Trues in each line
        previous_roots[new_seqs_place_bool] = new_seqs.reshape(-1)

        token_postn[already_finished_given_seqs] = (
            self.acceptance_rate_pad_for_alredy_finished_seqs
        )
    ...
    def get_vocab_tokens_bool_lib(self, draft_lib):
        ...
        vocab_tokens = (
            ...
            .unsqueeze(0)
            .unsqueeze(-1)
            .expand(b_sz, self.vocab_size, n_drafts)
        )
        # -> (b_sz, vocab_size, n_drafts)
        vocab_tokens_bool = draft_start_tokens.unsqueeze(1).expand(
            b_sz, self.vocab_size, n_drafts
        ) == vocab_tokens.type_as(draft_lib)
        # -> (b_sz, vocab_size, n_drafts)
    ...
    def generate(self, src: "torch.LongTensor") -> list["torch.LongTensor"]:
        ...
        memory = (
            memory.unsqueeze(1)
            .repeat(1, n_drafts, 1, 1)
            .reshape(b_size * n_drafts, src_len, emb_dim)
        )
        src_pad_mask = (
            src_pad_mask.unsqueeze(1)
            .repeat(1, n_drafts, 1)
            .reshape(b_size * n_drafts, src_len)
        )
        ...
        logits_base = torch.full(
            (b_size * n_drafts, draft_len + 1, self.vocab_size), 0.0, device=src.device
        )
        ...
        while (
            ...
            bool_idx_of_unfinished = ~(
                (generated_tokens == self.eos_token_idx).sum(-1).bool()
            )
            # -> (n_candidates)
            ...
            draft_place_bool_idx_input = draft_place_bool.unsqueeze(1).repeat(
                1, n_drafts, 1
            )
            # -> (b_s * bm_sz, n_drafts, drafted_len)
            generated_tokens_input = generated_tokens.unsqueeze(1).repeat(
                1, n_drafts, 1
            )
            # -> (b_s * bm_sz, n_drafts, drafted_len)

            generated_tokens_input[draft_place_bool_idx_input] = draft_tokens.reshape(
                -1
            )
            ...
            bool_idx_of_unfinished = (
                bool_idx_of_unfinished.unsqueeze(-1)
                .repeat(1, n_drafts)
                .flatten(end_dim=1)
            )
            # -> (b_s * bm_sz * n_drafts)
            draft_place_bool_idx_input = draft_place_bool_idx_input[
                bool_idx_of_unfinished
            ]
            ...
            pred_logits = pred_logits[
                torch.logical_or(
                    draft_place_bool_idx_input,
                    torch.roll(draft_place_bool_idx_input, -1, 1),
                )
            ...
            masked_probs = mask_with_num_logits_according_nucleus(
                pred_logits,
                nucleus=0.9975,
                max_num_of_unmasked_positions=self.n_best,
                num="-inf",
            ...
            masked_probs = masked_probs.reshape(
                n_candidates, n_drafts, draft_len + 1, vocab_size
            )
            ...
            n_accepted_in_drafts = self.calculate_n_accepted_in_drafts(
                draft_tokens, masked_probs
            )
            ...
            chosen_drafts = torch.gather(
                draft_tokens,
                dim=1,
                index=draft_i.unsqueeze(-1).expand(n_candidates, 1, draft_len),
            ).squeeze(1)
            ...
            pred_logits = torch.gather(
                pred_logits,
                dim=1,
                index=draft_i.unsqueeze(-1)
                .unsqueeze(-1)
                .expand(n_candidates, 1, draft_len + 1, vocab_size),
            ).squeeze(1)
            #   -> (n_candidates, draft_len + 1, vocab_size)
    ...
    def calculate_n_accepted_in_drafts(self, draft_tokens, masked_probs):
        ...
        """
        draft_tokens_probs = torch.gather(
            masked_probs[:, :, :-1, :], dim=-1, index=draft_tokens.unsqueeze(-1)
        ).squeeze(-1)
        #   -> (n_candidates, n_drafts, draft_len)
        verification = draft_tokens_probs != 0.0

        _range = verification.cumsum(-1)  # (n_candidates, n_drafts, draft_len)
        accepted_in_drafts_bool = (
            torch.arange(1, verification.size(2) + 1)
            .unsqueeze(0)
            .unsqueeze(0)
            .type_as(_range)
            == _range
        ...
        return accepted_in_drafts_bool.sum(
            -1
        )  # (n_candidates, n_drafts, draft_len) -> (n_candidates, n_drafts)
...
class TranslationInferenceBeamSearchSpeculative:
    ...
    def generate_trying_all_the_drafts(
...
def mask_with_num_logits_according_nucleus(
    pred_logits, nucleus, max_num_of_unmasked_positions, num=0.0
):
    """
    ...
    sorted_logits, sorted_indices = torch.sort(
        pred_logits, descending=True
    )  # -> (n_candidates * curr_len, vocab_size)
    cumulative_probs = torch.cumsum(
        sorted_logits.softmax(-1), dim=-1
    )  # -> (n_candidates * curr_len, vocab_size)
    ...
    cumulative_probs[:, 0] = (
        nucleus - 1
    )  # this protects the best probability in each distribution
    ...
    keep_candidates_mask = (
        cumulative_probs < nucleus
    )  # -> (n_candidates * curr_len, vocab_size)

    keep_candidates_mask[:, max_num_of_unmasked_positions:] = (
        False  # no more than max_num_of_unmasked_positions
    )

    sorted_logits.masked_fill_(
        ~keep_candidates_mask, float(num)
    )  # the all unapproved tokens logits
    # will be set equal to float(num)

    masked_logits_according_nucleus = torch.gather(
        sorted_logits, 1, sorted_indices.argsort(1)
    )
    # -> (n_candidates * curr_len, vocab_size)
